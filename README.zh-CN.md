...
| 🚦 **状态灯** | 每个标签页和分屏都有灰 / 黄 / 绿三态点:空闲、执行中、已完成等你处理。由 OSC 133 shell 集成驱动,无集成时用输出静默启发式兜底。在 `claude` 这类 TUI 里(对 shell 而言是一条跑很久的命令),灯改看它的输出:在刷屏 = 在干活,静下来 = 该你了。分屏的标签页在标签栏上按各格的视觉顺序**一格一个点**,所以你能直接看出是**哪一格**在等你。 |
| 🔔 **完成提示音** | 分屏跑完的那一刻响一声,每几秒最多一次。铃比灯严格:有 shell 集成时只认程序**明确宣告**的完成,所以 TUI 只是在两次工具调用之间安静了一会儿的话,灯会转绿但不出声 —— 否则限流会变成整个任务时长里的节拍器。 |
...
| ↗️ **分屏跨标签页移动** | <kbd>⌘⌃1…8</kbd> 把当前格送到第 N 个标签页 —— 用的就是该标签页自己 <kbd>⌘N</kbd> 的数字,只差一个修饰键;<kbd>⌘⌃←→</kbd> 送到相邻标签页,<kbd>⌘⌃T</kbd> 移出成新标签页。shell 不中断、历史一起过去、状态灯也不会闪。把一个标签页的最后一格移走,那个标签页会随之消失 —— 于是这个操作同时也是「合并标签页」。 |
...
| ⚙️ **设置面板** | 一个齿轮搞定:切换**默认语言**(语音识别与整个界面一起切,English ⇆ 中文)、开关自动标点、设定回滚行数、开关会话历史、开关完成提示音、设定默认技能库文件夹、选择**命令输入颜色**,同一面板还有完整快捷键清单。 |
...
| | | | <kbd>⌘⌃←</kbd> / <kbd>⌘⌃→</kbd> | 把分屏送到上一个 / 下一个标签页 |
...
**app 内设置(⚙️)。** 默认语言、自动标点、回滚行数、会话历史、完成提示音、默认技能库文件夹、命令输入颜色都能在齿轮面板里直接改,无需编辑文件,且优先级高于配置文件。
...
- [x] 分屏跨标签页移动(shell 与历史不中断)、标签栏每格状态点、完成提示音
