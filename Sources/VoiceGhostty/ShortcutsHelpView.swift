    ...
    private var sections: [Section] {
        [
            ...
            Section(title: loc("Splits", "分屏"), items: [
                ...
                Item(keys: "⌘⌃1…⌘⌃8", desc: loc("Send this split to the Nth tab (shell and scrollback come along)",
                                                 "把当前分屏送到第 N 个标签页(shell 和历史一起过去)")),
                Item(keys: "⌘⌃← / ⌘⌃→", desc: loc("Send this split to the previous / next tab",
                                                   "把当前分屏送到上一个 / 下一个标签页")),
                Item(keys: "⌘⌃T", desc: loc("Send this split out to a tab of its own", "把当前分屏移出成新标签页")),
