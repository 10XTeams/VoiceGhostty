...
可选 `FoundationModels`(macOS 26+)。约 2.9k 行 Swift、19 个文件,跑起来不依赖任何外部服务。
...
| 绝不阻塞 | 服务没起、超时(15 秒)、解析失败 → 静默使用原文。矫正只能是锦上添花,绝不能吞掉用户说的话。 | `Command/OllamaClient.swift` |
...
- <kbd>⌘D</kbd> 向右分屏、<kbd>⌘⇧D</kbd> 向下分屏 —— 每个标签页最多 2 格。
- <kbd>⌘⌥←↑↓→</kbd> 按方向切换焦点(Ghostty `goto_split` 语义);与分屏轴垂直的方向忽略。
- 点击某一格,**语音、搜索、执行**全部路由到它 —— 焦点通过 `mouseDown` 捕获并同步回会话状态。
- 非活动分屏用半透明遮罩压暗,而不是画边框。
- 分屏时每格各自显示状态点,一眼看出哪边在跑、哪边跑完了。
...
- 你正在使用的那个单独终端保持中性 —— 终端不该用自己的灯来烦自己。分屏时两格各自独立报告。
- 每 0.5 秒轮询一次,同时刷新工作目录 —— 标签标题就是靠它保持最新的。

源文件:`Terminal/SessionStore.swift`、`Terminal/TerminalController.swift`
...
`~/.config/voiceghostty/config`,极简 `key = value`,`#` 注释,未知键和坏行直接忽略(不报错)。**共 12 个键:**
