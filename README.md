...
| ⚙️ **Settings panel** | One gear button: switch the **default language** (flips *both* speech recognition and the whole UI, English ⇆ 中文), toggle automatic punctuation, set scrollback lines, turn session history on or off, set the default skill-library folder, and pick the **command-input color** — with the full shortcut reference on the same panel. |
...
**In-app Settings (⚙️).** The default language, automatic punctuation, scrollback lines, session history, default skill-library folder, and command-input color are also editable in the gear panel — no file editing needed, and these win over the config file.
